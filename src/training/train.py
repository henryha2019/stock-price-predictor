...
def _coerce_all_numeric_float64(df: pd.DataFrame) -> pd.DataFrame:
    """
    MLflow enforces input schema strictly. Ensure every numeric column is float64
    so columns like `volume` cannot remain int64 and break pyfunc.predict().
    """
    df = df.copy()
...
def _coerce_X_float64(X: pd.DataFrame) -> pd.DataFrame:
    """
    Extra safety for slices (X.iloc[...]) which may preserve int dtypes.
    """
    X = X.copy()
    for c in X.columns:
...
def main() -> None:
    ...
    # IMPORTANT: make all numeric columns float64 (prevents MLflow schema failures)
    ...
    # Inject feature metadata into cfg so every model has feature_cols available
    cfg = dict(cfg)
    cfg["features"] = {
    ...
    if n_splits < 2:
        # Avoid sklearn crash: "n_splits=2 or more" (you hit this already)
        n_splits = 2
    ...
    with mlflow.start_run(run_name=args.model_family):
        ...
        for i, (tr_idx, va_idx) in enumerate(tscv.split(X)):
            ...
            # CRITICAL: enforce float64 on slices (prevents volume int64 -> schema error)
            X_tr = _coerce_X_float64(X_tr)
            X_va = _coerce_X_float64(X_va)
            ...
            # CRITICAL: enforce float64 again right before predict
            pred = pyfunc.predict(_coerce_X_float64(X_va))
        ...
        X_train = _coerce_X_float64(X_train)
        X_val = _coerce_X_float64(X_val)
